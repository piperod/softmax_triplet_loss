...
def generate_tsne(activations):
    mds = MDS( n_components=2,random_state=42, max_iter=tsne_iter)
    X_2d = mds.fit_transform(activations)
    X_2d -= X_2d.min(axis=0)
    X_2d /= X_2d.max(axis=0)
    return X_2d

def save_tsne_grid(img_collection,labels, X_2d, out_res, out_dim,out_dir,out_name='tsne_visualization.png'):
...
import random
def main(dataset,activations_path,out_dim,out_res,out_dir,out_name):
    ...
    save_tsne_grid(img_collection,labels[:np.square(out_dim)], X_2d, out_res, out_dim,out_dir,out_name)
...
if __name__ == '__main__':
    ...
    perplexity = args.per
    tsne_iter = args.iter
    out_name = 'mds_visualization_%05d.png'%tsne_iter
    ...
    main(dataset,pooling_path,out_dim,out_res,features_path,out_name)